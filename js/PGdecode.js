...
// andy.hine trim spaces and line breaks and preserve text around encrypted message
// replace chars in private key with those from password
if (text.indexOf('-----BEGIN PGP PRIVATE KEY BLOCK') == -1) {
	var startIndex = text.indexOf('-----BEGIN PGP MESSAGE');
	var endIndex = text.indexOf('END PGP MESSAGE-----') + 'END PGP MESSAGE-----'.length;
	//alert(startIndex);
	//alert(endIndex);
	if (startIndex > 0) {
		appendText = text.slice(0, startIndex);
		//alert(appendText);
	}	
	if (endIndex < text.length) {
		prependText = text.slice(endIndex, text.length);
		//alert(prependText);
	}
	...
	// The private key has been passed
	pass = window.prompt("Password:", "");

	// Replace missing chars in private key with those from my password
	text = text.replace('SAfZFo', 'SAfZFo' + pass.substring(1, 3));
...
//     r+='---Public Key in Base64---\n'+s2r(s.substr(k,lm+le+4))+'\n---\n';
 ...
 return appendText + text + prependText;
